...
#     try:
#         attendance_record = AttendanceRecord.objects.filter(employee_id__employee_id=employee_id).latest('login_time')
#     except AttendanceRecord.DoesNotExist:
#         return Response({"error": "Attendance record not found"}, status=status.HTTP_404_NOT_FOUND)
    
#     attendance_record.logout_work_location = logout_work_location
#     attendance_record.logout_time = timezone.now()
...
def log_employee_check_out(request, employee_id):
    ...
    try:
        attendance_record = AttendanceRecord.objects.filter(employee_id=employee).latest('login_time')
    except AttendanceRecord.DoesNotExist:
        return Response({"error": "Attendance record not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    if serializer.is_valid():
        ...
        attendance_records = AttendanceRecord.objects.filter(employee_id=employee, login_time__date=attendance_record.login_time.date())
        total_hours, is_present = calculate_work_hours_and_status(attendance_records)
